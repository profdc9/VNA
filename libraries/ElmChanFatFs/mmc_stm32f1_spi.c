...
void write_message(const char *c);
...
#define FCLK_FAST() { SPIx_CR1 = (SPIx_CR1 & ~0x38) | 0x08; }	/* Set SCLK = PCLK / 4 */
...
#elif SPI_CH == 2	/* PB9:MMC_CS1, PB8:MMC_CS2, PB13:MMC_SCLK, PB14:MMC_DO, PB15:MMC_DI, PD8:MMC_CD */
#define CS_HIGH()	GPIOB_BSRR = curdrv ? _BV(8) : _BV(9)
#define CS_LOW()	GPIOB_BSRR = curdrv ? _BV(8+16) : _BV(9+16)
...
#define	SPIxENABLE() {\
	...
	__enable_peripheral(IOPBEN);\
	__gpio_conf_bit(GPIOB, 9, OUT_PP);							/* PB9: MMC_CS */\
	__gpio_conf_bit(GPIOB, 8, OUT_PP);							/* PB8: MMC_CS */\
...
#elif SPI_CH == 3	/* PA4:MMC_CS, PB13:MMC_SCLK, PB14:MMC_DO, PB15:MMC_DI, PD8:MMC_CD */
#define CS_HIGH()	GPIOA_BSRR = _BV(4)
#define CS_LOW()	GPIOA_BSRR = _BV(4+16)
#define	MMC_CD		1  /* Card detect (yes:true, no:false, default:true) */
#define	MMC_WP		0 /* Write protected (yes:true, no:false, default:false) */
...
)
{
	...
	if (drv > 1) return STA_NOINIT;			/* Supports only drive 0 */
	curdrv = drv;
	...
	if (send_cmd(CMD0, 0) == 1) {			/* Put the card SPI/Idle state */
		...
		if (send_cmd(CMD8, 0x1AA) == 1) {	/* SDv2? */
			...
			{
				disk_timerproc();		/* Wait for end of initialization */
			};
...
)
{
	...
	if ((drv>1) || !count) return RES_PARERR;		/* Check parameter */
	curdrv = drv;
...
)
{
	...
	if (drv > 1) return RES_PARERR;					/* Check parameter */
	curdrv = drv;
	if (Stat & STA_NOINIT) return RES_NOTRDY;	/* Check if drive is ready */
...
extern volatile DWORD systick_uptime_millis;
/* you should use millis() here if it's available at a function */
/*-----------------------------------------------------------------------*/
...
void disk_timerproc (void)
{
	...
	if (last == systick_uptime_millis) return;
	last = systick_uptime_millis;
