...
void touchscreen_allocate_axis_data(const char *save_header, const char *format_string, bool polar, bool twotrace)
{
...
void touchscreen_axis_clear_bottom(void)
{
  tft.fillRect(0, tft.height() - TOUCHSCREEN_BOTTOM_LINES, tft.width(), TOUCHSCREEN_BOTTOM_LINES, ILI9341_BLACK);
  tft.setTextSize(1);
  tft.setTextColor(ILI9341_WHITE);
}

const touchscreen_button_panel_entry touchscreen_save_or_quit_panel[] =
{
  { 0, 30, 216, 4, 60, 20, 0xFFFF, 0x0000, 0xFFFF, "Cont", 1, NULL },
  { 1, 130, 216, 4, 60, 20, 0xFFFF, 0x0000, 0xFFFF, "Save", 1, NULL },
  { 2, 230, 216, 4, 60, 20, 0xFFFF, 0x0000, 0xFFFF, "Quit", 1, NULL }
};

int touchscreen_select_save_or_quit()
{
  int n;
  touchscreen_axis_clear_bottom();
  touchscreen_draw_button_panel(&tft, sizeof(touchscreen_save_or_quit_panel) / sizeof(touchscreen_button_panel_entry), touchscreen_save_or_quit_panel);
  touchscreen_solid_release();
  do
  {
    n = touchscreen_get_button_press(&tft, sizeof(touchscreen_save_or_quit_panel) / sizeof(touchscreen_button_panel_entry), touchscreen_save_or_quit_panel);
  } while (n < 0);
  touchscreen_axis_clear_bottom();
  return n;
}
...
float touchscreen_axis_position(int n)
{
  return ((taps.maxx - taps.minx) * n) / ((float)taps.num_axis_data) + taps.minx;
}

void touchscreen_save(Complex *axis_data)
{  
  touchscreen_axis_clear_bottom();
  tft.setCursor(50,220);
  FIL fp;
  char filename[15];
  if (!opendatafile(&fp,filename))
  {
      tft.print("Could not save data");
  } else
  {
    UINT bw;
    f_write(&fp, taps.save_header, strlen(taps.save_header), &bw);
    for (int n=0;n<taps.num_axis_data;n++)
    {
      int len;
      char s[80];
      float xaxis = touchscreen_axis_position(n);
      Complex temp = axis_data[n];
      temp = taps.polar ? touchscreen_rec2polar(temp) : temp;
      len = mini_snprintf(s, sizeof(s)-1, "%f,%03f,%03f\n",float2int32(xaxis),float2int32(temp.real), float2int32(temp.imag));
      f_write(&fp, s, len, &bw);
    }
    f_close(&fp);
    tft.print("Saved as");
...
bool touchscreen_get_touch_axis(bool axis_buffer, bool redobottom)
{
  ...
  if ((taps.current_selected >= 0) && (!redobottom))
  {
    ...
    if (jogwheel.getSelect(true))
    {
      int n = touchscreen_select_save_or_quit();
      if (n == 2) return true;
      if (n == 1) touchscreen_save(axis_data);
    } 
  ...
  touchscreen_axis_clear_bottom();
  float xaxis = touchscreen_axis_position(taps.current_selected);
  tft.setCursor(0, h + 8);
...
const char *swr_save_header = "Frequency,SWR,Null\n";
...
int touchscreen_swr(int code, void *v)
{
  ...
  touchscreen_allocate_axis_data(swr_save_header,"%f: SWR %02f", false, false);
...
const char *zacq_port1_header = "Frequency,Z Reflected Real, Z Reflected Imag\n";
const char *zacq_port2_header = "Frequency,Z Transmit Real, Z Transmit Imag\n";
...
int touchscreen_zacq(int code, void *v)
{
  ...
  touchscreen_allocate_axis_data(code == 201 ? zacq_port2_header : zacq_port1_header, "%f: Z(%03f,%03f)", false, true);
...
const char *s11_header = "Frequency,S11 mag,S11 angle\n";
const char *s21_header = "Frequency,S21 mag,S21 angle\n";
...
int touchscreen_sparm(int code, void *v)
{
  ...
  touchscreen_allocate_axis_data(code == 301 ? s21_header : s11_header, "%f: %03f dB %03f degs", touchscreen_smith_chart, !touchscreen_smith_chart);
...
{
  { 200, 0, 0,   4, 0, 36, 0xFFFF, 0x0000, 0xFFFF, "Char Imped", 2, touchscreen_char_impedance },
  { 300, 160, 0,   4, 0, 36, 0xFFFF, 0x0000, 0xFFFF, "Averages", 2, touchscreen_averages },
  { 400, 0, 50, 4, 0, 36, 0xFFFF, 0x0000, 0xFFFF, "Imped Scale", 2, touchscreen_impedance_scale },
  { 500, 160, 50, 4, 0, 36, 0xFFFF, 0x0000, 0xFFFF, "dB Scale", 2, touchscreen_db_scale },
  { 600, 0, 100,  4, 0, 36, 0xFFFF, 0x0000, 0xFFFF, "Smith", 2, touchscreen_smith },
  { 700, 80, 100,  4, 0, 36, 0xFFFF, 0x0000, 0xFFFF, "Atten", 2, touchscreen_atten },
  { 800, 160, 100,  4, 0, 36, 0xFFFF, 0x0000, 0xFFFF, "Series/Shunt", 2, touchscreen_series },
  { 900, 0, 150,  4, 0, 36, 0xFFFF, 0x0000, 0xFFFF, "Serial Rem", 2, touchscreen_remote },
  { 1000, 160, 150,  4, 0, 36, 0xFFFF, 0x0000, 0xFFFF, "Cal Freq", 2, touchscreen_calfreqs },
...
const touchscreen_button_panel_entry calbuttonpanel[] =
{
  { 400, 10, 20, 4, 0, 36, 0xFFFF, 0x0000, 0xFFFF, "Open", 2, touchscreen_open },
  { 500, 160, 20, 4, 0, 36, 0xFFFF, 0x0000, 0xFFFF, "Short", 2, touchscreen_short },
  { 600, 10, 80, 4, 0, 36, 0xFFFF, 0x0000, 0xFFFF, "Load", 2, touchscreen_load },
  { 700, 160, 80, 4, 0, 36, 0xFFFF, 0x0000, 0xFFFF, "Thru", 2, touchscreen_thru },
  { 900, 10, 140, 4, 0, 36, 0xFFFF, 0x0000, 0xFFFF, "ReadCal", 2, touchscreen_readcal },
  { 1000, 160, 140, 4, 0, 36, 0xFFFF, 0x0000, 0xFFFF, "WriteCal", 2, touchscreen_writecal },
  { 0, 10, 200, 4, 0, 36, 0xFFFF, 0x0000, 0xFFFF, "Return to Main Menu", 2, NULL }
};

int touchscreen_cal(int code, void *v)
{
  bool panelback = false;
  for (;;)
  {
    if (!panelback)
    {
      panelback = true;
      tft.fillScreen(ILI9341_BLACK);
      touchscreen_draw_button_panel(&tft, sizeof(calbuttonpanel) / sizeof(touchscreen_button_panel_entry), calbuttonpanel);
    }
    int16_t cd = touchscreen_get_button_press(&tft, sizeof(calbuttonpanel) / sizeof(touchscreen_button_panel_entry), calbuttonpanel);
    if (cd == 0) break;
    if (cd >= 0) panelback = false;
  }
}

int touchscreen_mass(int code, void *v)
{
  bool masschoice;
  if (!touchscreen_enter_yesno("Enter Mass Storage?", NULL, masschoice)) return 0;
  if (masschoice == 0) return 0;
  touchscreen_display_message("Mass Storage Mode\nPower cycle to leave");
  vna_mass_storage_device();
}

const touchscreen_button_panel_entry mainpanel[] =
{
  { 100, 10, 30,   4, 0, 36, 0xFFFF, 0x0000, 0xFFFF, "Freq", 2, touchscreen_freqs },
  { 1200, 78, 30, 4, 0, 36, 0xFFFF, 0x0000, 0xFFFF, "Settings", 2, touchscreen_settings },
  { 1300, 195, 30, 4, 0, 36, 0xFFFF, 0x0000, 0xFFFF, "SWR", 2, touchscreen_swr },
  { 1400, 250, 30, 4, 0, 36, 0xFFFF, 0x0000, 0xFFFF, "Spot", 2, touchscreen_spot },
  { 200, 10, 80,   4, 0, 36, 0xFFFF, 0x0000, 0xFFFF, "ZRef", 2, touchscreen_zacq },
  { 201, 110, 80,   4, 0, 36, 0xFFFF, 0x0000, 0xFFFF, "ZThru", 2, touchscreen_zacq },
  { 300, 195, 80, 4, 0, 36, 0xFFFF, 0x0000, 0xFFFF, "S11", 2, touchscreen_sparm },
  { 301, 250, 80, 4, 0, 36, 0xFFFF, 0x0000, 0xFFFF, "S21", 2, touchscreen_sparm },
  { 400, 10, 130, 4, 0, 36, 0xFFFF, 0x0000, 0xFFFF, "Calib", 2, touchscreen_cal },
  { 1200, 120, 130, 4, 0, 36, 0xFFFF, 0x0000, 0xFFFF, "Mass", 2, touchscreen_mass },
  { 1300, 195, 130,   4, 0, 36, 0xFFFF, 0x0000, 0xFFFF, "Sweep", 2, touchscreen_sweep_set },
